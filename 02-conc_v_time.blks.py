...
fig.savefig('02-00-O_2_conc_v_time_Z=-30.png',dpi=500,transparent=True,bbox_inches='tight')
...
fig.set_size_inches(7,4)
fig.savefig('02-01-O_2_conc_v_time_Z=-30.2blk.png',dpi=500,transparent=True,bbox_inches='tight')


################################################################################
# Plot [O2] at Z=-30 Angstrom vs time with blocks of arbitrary size
####

# Specify block size (must be integer)
blk_size = 2

# Calculate blocks
blkd_data = np.full((data_len-1)/blk_size,-1.)

print('Initial blkd_data values: {} {} ...'.format(blkd_data[0],blkd_data[1]))

for it1 in range(0,(data_len-1)/blk_size):
    _it = it1*blk_size
    blkd_data[it1] = np.mean(data01[(_it)+1:(_it)+1+(blk_size-1)+1,91])
#                                         ^       ^    ^        ^ excl. range
#                                         |       |    |
#                                         |       |    +-- for block range
#                                         |       | 
...
print('Computed blkd_data values: {} {} ...'.format(blkd_data[0],blkd_data[1]))

# Create time data for blocks
blkd_time = np.full((data_len-1)/blk_size,-1)

for it1 in range(0,len(blkd_time)):
    blkd_time[it1] = 2*it1

print('blkd_time values: {} {} ...'.format(blkd_time[0],blkd_time[1]))
...
ax.plot(blkd_time/100.,blkd_data,alpha=0.85,color='b',linewidth=0.5)

# Title
ax.set_title('[O$_2$] at Z=-30$\AA{}$ vs Time Blocksize=arb',fontname='Times New Roman',fontweight='bold',fontsize=14)
...
fig.set_size_inches(7,4)
fig.savefig('02-02-O_2_conc_v_time_Z=-30.arbblk.png',dpi=500,transparent=True,bbox_inches='tight')
